...
generate
    `ifdef RAM_IMPL
    if (`RAM_IMPL == pp_pkg::ImplGeneric) begin : g_generic
        prim_ram_2p #(
            .Width(32),
            .Depth(Depth),
            .DataBitsPerMask(8),
    ...
    end else if (`RAM_IMPL == pp_pkg::ImplQuartus) begin : g_quartus
        prim_altera_ram_2p #(
            .Depth          (Depth),
            .MemInitFile    (MemInitFile)
         ) prim_altera_ram_2p (
    ...
    end else begin : g_failure
        $fatal("RAM_IMPL has an invalid definition");
    end
    `else
        $fatal("RAM_IMPL is not defined");
    `endif
endgenerate
