...
class obi_driver extends uvm_driver #(obi_item);
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db #(virtual interface obi_if)::get(null, "*", "obi_if", iface))
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            // address phase
            iface.drv.drv_cb.req   <= 1'b1;
            iface.drv.drv_cb.addr  <= obi_packet.addr;
            iface.drv.drv_cb.wdata <= obi_packet.wdata;
            ...
            iface.drv.drv_cb.wdata <= '0;
            iface.drv.drv_cb.we    <= 1'b0;
            iface.drv.drv_cb.be    <= '0;

            // response phase (both reads and writes complete on rvalid)
            do @(iface.drv.drv_cb); while (iface.drv.drv_cb.rvalid !== 1'b1);
            if (!obi_packet.we) begin
                obi_packet.rdata = iface.drv.drv_cb.rdata;
                obi_packet.err   = iface.drv.drv_cb.err;
            end
